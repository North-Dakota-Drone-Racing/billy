...
async def set_serverConfiguration(discord_serverId:int, discord_channelId:int, mgp_apikey:str) -> dict:
...
async def get_serverInfo(discord_id:int) -> db_types.chapter:
    ...
    async with aiosqlite.connect(database_file) as db:
        rows = await db.execute("SELECT * FROM chapter WHERE discord_serverId=?", (discord_id,))
...
async def get_chapterServers(chapter_id) -> list[db_types.chapter]:
    ...
    async with aiosqlite.connect(database_file) as db:
        chapters = await db.execute("SELECT * FROM chapter WHERE mgp_chapterId=?", (chapter_id,))
        servers = [db_types.chapter(*chapter) async for chapter in chapters]
...
async def remove_Races(race_ids:list[str]) -> None:
    ...
    async with aiosqlite.connect(database_file) as db:
        await asyncio.gather(*[db.execute("DELETE FROM race WHERE mgp_raceId=?", (race_id,)) for race_id in race_ids])
        await db.commit()
